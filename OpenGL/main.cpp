...
"void main()\n"
"{\n"
"   FragColor = vec4(0.0f, 0.0f, 1.0f, 1.0f);\n" // RGBA: here i changed the color of the recatngular into blue
"}\n\0";
...
int main()
{
    ...
    // --- 6. تعريف البيانات (مستطيل) ---
    float vertices[] = {
        //المثلث الأول
        -0.6f, -0.3f, 0.0f,  // أسفل يسار
         0.6f, -0.3f, 0.0f,  // أسفل يمين
         0.6f,  0.3f, 0.0f,  // أعلى يمين

        // المثلث الثاني
        -0.6f, -0.3f, 0.0f,  // أسفل يسار
         0.6f,  0.3f, 0.0f,  // أعلى يمين
        -0.6f,  0.3f, 0.0f   // أعلى يسار
    };// i have made two triangles each in different dimensions to create a rectangular
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glClearColor(1.0f, 1.0f, 1.0f, 1.0f); // and here the background color is white
        glClear(GL_COLOR_BUFFER_BIT);

        // ج. الرسم
        glUseProgram(shaderProgram); // تفعيل الشيدر
        glBindVertexArray(VAO); // تفعيل كائن المثلث
        glDrawArrays(GL_TRIANGLES, 0, 6); // we have 6 vertexes because we are drawing two traingles to make a rectangular
